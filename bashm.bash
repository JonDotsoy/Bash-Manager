...
function log {
    ...
    if [[ "${TYPE}" == "WARN" ||  "${TYPE}" == "WAR" ||  "${TYPE}" == "WARN" || "${TYPE}" == "WARNNING" ]]; then
        TYPECOLOR="${COLORWARN}"
        TYPE="WARN"
    fi
    ...
    if [[ "${LOG_DECORATE}" == "false" ]]; then
        echo -e ${LOG_PREFIX}'['${TYPE}']: '${MESSAGE}''${LOG_SUFFIX}
    else
        echo -e ${LOG_PREFIX}${TYPECOLOR}'['${TYPE}']:'${MESSAGECOLOR}' '${MESSAGE}''${LOG_SUFFIX}
    fi
}
...
function bashm {
  ...
  if [[ "${action}" == "version" || "${action}" == "-v" ]]; then
    echo "BashM, Version Develop (GNU bash)"
    echo "Copyright (C) 2015, Jon Dotsoy <hi@jon.soy> (http://jon.soy/)."
        ...
        # Default Instalations alias ls and theme
        ...
        if [[ ! -f "${BASHM_PATH}/plugin/${name_Plugin}.bash" || "${forseInstall}" == "force" ]]; then
            ...
            if [[ $CORRET_DOWNLOAD = true ]]
                then
                #statements
                log ok "Download ${name_Plugin} ok."
                bashm import ${name_Plugin} || CORRET_DOWNLOAD=false
            fi

            if [[ "${CORRET_DOWNLOAD}" == "false" ]]; then
                log err "Error to Download \"${name_Plugin}\" Plugin."
